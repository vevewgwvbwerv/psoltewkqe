...
-- === НОВАЯ ФУНКЦИЯ: ЗАМЕНА ПИТОМЦА В РУКЕ (КАК PetScaler_v3.226.lua) ===
local function replaceHandPetWithAnimation()
    print("\n✋ === ЗАМЕНА ПИТОМЦА В РУКЕ (КАК PetScaler_v3.226.lua) ===")
    ...
    print("🎯 Найден питомец в руке:", handPet.Name)
    print("🎯 Найден UUID питомец:", petModel.Name)
    
    -- Шаг 3: Создать копию UUID питомца (точно как в PetScaler_v3.226.lua)
    local petCopy = deepCopyModel(petModel)
    if not petCopy then
        print("❌ Не удалось создать копию!")
        return false
    ...
    -- Шаг 4: Заменить питомца в руке на копию (сохраняя позицию)
    local handPetCFrame = handPet.PrimaryPart and handPet.PrimaryPart.CFrame or handPet:GetModelCFrame()
    
    -- Удаляем старого питомца
    handPet:Destroy()
    
    -- Размещаем копию в Handle
    petCopy.Parent = handle
    if petCopy.PrimaryPart then
        petCopy:SetPrimaryPartCFrame(handPetCFrame)
    end
    
    print("✅ Питомец в руке заменен на UUID копию!")
    ...
    if next(originalMotors) and next(copyMotors) then
        ...
        print("🔥 Копия в руке теперь живая - как в PetScaler_v3.226.lua!")
