    ...
    for _, part in ipairs(parts) do
        ...
        tween.Completed:Connect(function()
            ...
            if completedTweens == #parts then
                ...
                -- КРИТИЧЕСКИ ВАЖНО: Принудительная коррекция ориентации после масштабирования
                print("🔧 Корректирую ориентацию копии чтобы она стояла как оригинал...")
                
                -- Находим оригинальную модель для копирования ориентации
                ...
                if originalModel and originalModel.PrimaryPart and model.PrimaryPart then
                    local originalCFrame = originalModel.PrimaryPart.CFrame
                    local copyPosition = model.PrimaryPart.Position
                    
                    -- Копируем ТОЛЬКО ориентацию оригинала, но сохраняем позицию копии
                    local correctedCFrame = CFrame.new(copyPosition) * (originalCFrame - originalCFrame.Position)
                    model:SetPrimaryPartCFrame(correctedCFrame)
                    
                    print("✅ Ориентация скорректирована - копия теперь стоит как оригинал!")
                else
                    print("⚠️ Не удалось найти оригинал для коррекции ориентации")
                end
            end
        end)
