...
-- Функция плавного масштабирования модели (ИСПРАВЛЕНО: с правильным Anchored управлением)
local function scaleModelSmoothly(model, scaleFactor, tweenTime)
    ...
    -- ИСПРАВЛЕНО: Сохраняем состояния Anchored и временно отключаем для синхронного масштабирования
    local anchoredStates = {}
    local anchoredPart = nil
    
    -- Находим основную заякоренную часть
    if model:FindFirstChild("RootPart") then
        anchoredPart = model.RootPart
    elseif model.PrimaryPart then
        anchoredPart = model.PrimaryPart
    elseif model:FindFirstChild("Torso") then
        anchoredPart = model.Torso
    elseif model:FindFirstChild("HumanoidRootPart") then
        anchoredPart = model.HumanoidRootPart
    end
    
    -- Сохраняем и временно отключаем Anchored для всех частей
    for _, part in ipairs(parts) do
        anchoredStates[part] = part.Anchored
        part.Anchored = false -- Временно отключаем для синхронного масштабирования
    end
    print("⚓ Временно отключен Anchored для синхронного масштабирования")
    ...
    for _, part in ipairs(parts) do
        ...
        tween.Completed:Connect(function()
            ...
            if completedTweens == #parts then
                ...
                -- ИСПРАВЛЕНО: Восстанавливаем правильные Anchored состояния после масштабирования
                print("⚓ Восстанавливаю правильные Anchored состояния...")
                for _, part in ipairs(parts) do
                    if part == anchoredPart then
                        part.Anchored = true  -- Только основная часть заякорена
                        print("⚓ Заякорена основная часть:", part.Name)
                ...
                print("✅ Anchored состояния восстановлены для анимации!")
            end
        end)
