...
local function main()
    ...
    -- Шаг 5: 🎭 ПОИСК И ЗАПУСК IDLE АНИМАЦИИ НА КОПИИ
    print("\n🎭 === ПОИСК IDLE АНИМАЦИИ ДЛЯ КОПИИ ===")
    
    -- Функция поиска и запуска idle анимации на копии
    local function findAndStartIdleOnCopy()
        -- Находим Animator в копии
        ...
        -- Находим Humanoid в копии
        local copyHumanoid = petCopy:FindFirstChildOfClass("Humanoid")
        if not copyHumanoid then
            print("❌ Humanoid не найден в копии!")
            return false
        end
        
        print("✅ Нашел Animator и Humanoid в копии")
        
        -- Останавливаем копию
        copyHumanoid.WalkSpeed = 0
        copyHumanoid.JumpPower = 0
        copyHumanoid.PlatformStand = true
        
        -- Останавливаем оригинал для поиска idle
        ...
        if originalHumanoid then
            ...
            print("🛑 Остановил оригинал для поиска idle")
        end
        
        -- Ждем перехода в idle
        print("⏳ Жду 5 секунд для перехода в idle...")
        wait(5)
        
        -- Ищем idle анимацию в оригинале
        ...
        local animationTracks = originalAnimator:GetPlayingAnimationTracks()
        print("🔍 Найдено анимаций в оригинале:", #animationTracks)
        
        local idleAnimation = nil
        for _, track in pairs(animationTracks) do
            if track.IsPlaying then
                local animName = track.Animation.Name:lower()
                print("🎬 Проверяю анимацию:", animName)
                
                if animName:find("idle") or animName:find("stand") or animName:find("breath") then
                    idleAnimation = track.Animation
                    print("✨ НАШЕЛ IDLE анимацию!", animName)
                    break
                end
            end
        end
        
        -- Если не нашли idle, берем любую кроме ходьбы
        if not idleAnimation then
            print("⚠️ Idle не найден, ищу любую не-ходьбу...")
            for _, track in pairs(animationTracks) do
                if track.IsPlaying then
                    local animName = track.Animation.Name:lower()
                    if not (animName:find("walk") or animName:find("run") or animName:find("move")) then
                        idleAnimation = track.Animation
                        print("✨ Нашел не-ходьбу:", animName)
                        break
                    end
                end
            end
        ...
        if not idleAnimation then
            print("❌ Никаких подходящих анимаций не найдено!")
            return false
        end
        
        -- Загружаем анимацию на копию
        local copyIdleTrack = copyAnimator:LoadAnimation(idleAnimation)
        if not copyIdleTrack then
            print("❌ Не удалось загрузить анимацию на копию!")
            return false
        end
        
        -- Настраиваем как бесконечную
        copyIdleTrack.Looped = true
        copyIdleTrack.Priority = Enum.AnimationPriority.Action
        
        -- Запускаем!
        copyIdleTrack:Play()
        print("🎭 ЗАПУСТИЛ IDLE АНИМАЦИЮ НА КОПИИ!")
        
        return true
    end
    
    -- Запускаем поиск и запуск idle анимации
    local idleSuccess = findAndStartIdleOnCopy()
    
    if idleSuccess then
        print("✅ Idle анимация найдена и запущена на копии!")
        print("🎭 Копия должна АНИМИРОВАТЬСЯ idle!")
    else
        print("⚠️ Не удалось запустить idle, запускаю живое копирование...")
        local connection = startLiveMotorCopying(petModel, petCopy)
    end
    
    -- Шаг 6: КОНТРОЛЬ ОРИГИНАЛЬНОГО ПИТОМЦА (ОПЦИОНАЛЬНО)
    print("\n🎯 === КОНТРОЛЬ ОРИГИНАЛА (ОПЦИОНАЛЬНО) ===") 
