...
local function replaceHandPetWithAnimation()
    ...
    -- Шаг 5: LIVE КОПИРОВАНИЕ АНИМАЦИЙ ОТ ПИТОМЦА В РУКЕ (точно как в PetScaler_v3.226.lua)
    print("\n🎭 === LIVE КОПИРОВАНИЕ АНИМАЦИЙ ОТ ПИТОМЦА В РУКЕ ===")
    print("🔄 Копирую анимации от питомца в руке к копии в Workspace...")
    
    -- Функция получения анимируемых частей из Tool (исключая Handle)
    ...
    -- Получаем части питомца в руке для копирования анимаций
    local handPetParts = getAnimatedPartsFromTool(handTool)
    print("📦 Анимируемых частей в руке:", #handPetParts)
    
    if #handPetParts > 0 then
        -- LIVE копирование CFrame состояний (точно как в PetScaler_v3.226.lua)
        local previousCFrameStates = {}
        local INTERPOLATION_SPEED = 0.3
        local POSITION_THRESHOLD = 0.001
        local ROTATION_THRESHOLD = 0.001
        
        local directConnection = RunService.Heartbeat:Connect(function()
            local appliedCount = 0
            local changesDetected = 0
            
            -- Копируем CFrame изменения от питомца в руке к копии
            for _, handPart in ipairs(handPetParts) do
                if handPart and handPart.Parent then
                    ...
                    local copyPart = findCorrespondingPart(petCopy, partName)
                    if copyPart then
                        local success, errorMsg = pcall(function()
                            -- Применяем с интерполяцией для плавности (только к незаякоренным частям)
                            if not copyPart.Anchored and copyPart.Parent then
                                copyPart.CFrame = copyPart.CFrame:Lerp(currentCFrame, INTERPOLATION_SPEED)
                                appliedCount = appliedCount + 1
                            end
                        end)
                        
                        if not success then
                            print("⚠️ Ошибка применения CFrame:", errorMsg)
                        end
                    end
                end
            end
        ...
        print("✅ LIVE копирование анимаций от питомца в руке запущено!")
        print("🎭 Копия следует за движениями питомца в руке!")
        print("🔥 Точно как в PetScaler_v3.226.lua - два питомца с синхронными анимациями!")
        
        return true
    else
        print("⚠️ Анимируемые части в руке не найдены")
        return false
    end
end
