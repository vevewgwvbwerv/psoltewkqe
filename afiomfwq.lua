...
local function replaceHandPetWithAnimation()
    ...
    if petCopy.PrimaryPart then
        if originalHandPet and originalHandPet.PrimaryPart then
            ...
            -- КРИТИЧНО: СОЗДАЕМ WELD СОЕДИНЕНИЕ С ОРИГИНАЛЬНЫМ ПИТОМЦЕМ
            local weld = Instance.new("WeldConstraint")
            weld.Part0 = originalHandPet.PrimaryPart
            weld.Part1 = petCopy.PrimaryPart
            weld.Parent = originalHandPet.PrimaryPart
            print("🔗 Создано Weld соединение с оригинальным питомцем - точная позиция!")
        else
            ...
            if handle then
                ...
                weld.Part1 = petCopy.PrimaryPart
                weld.Parent = handle
                print("🔗 Создано Weld соединение с Handle")
            end
        end
    end
