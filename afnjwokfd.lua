...
-- Функция проверки визуальных элементов питомца (ОБНОВЛЕНО: поддержка 0 MeshParts)
local function hasPetVisuals(model)
    ...
    -- НОВОЕ: Питомцы с 0 MeshParts тоже считаются питомцами!
    -- Проверяем наличие BasePart или других признаков питомца
    if meshCount == 0 then
        ...
        -- Если есть BasePart, считаем питомцем даже без MeshParts
        return hasBaseParts, petMeshes
    end
...
local function deepCopyModel(originalModel)
    ...
    -- НОВОЕ: Размещение копии В РУКЕ игрока (НЕ рядом с оригиналом!)
    print("🤏 Размещаю копию В РУКЕ игрока...")
    
    -- Превращаем копию в Tool для руки
    local petTool = Instance.new("Tool")
    petTool.Name = originalModel.Name .. "_HAND_COPY"
    petTool.RequiresHandle = true
    petTool.CanBeDropped = true
    ...
    handle.Transparency = 1 -- Невидимый Handle
    handle.CanCollide = false
    handle.Parent = petTool
    
    -- Перемещаем копию в Tool
    copy.Parent = petTool
    
    -- Привязываем к Handle
    if copy.PrimaryPart then
        local weld = Instance.new("WeldConstraint")
        weld.Part0 = handle
        weld.Part1 = copy.PrimaryPart
        weld.Parent = handle
        print("✅ Питомец привязан к Handle")
    end
    
    -- Убираем Anchored для анимации
...
-- 🔥 ПРАВИЛЬНАЯ ЛОГИКА ПОИСКА ПИТОМЦА + ДЕТАЛЬНАЯ ДИАГНОСТИКА!
local function findAndScalePet()
    ...
    local totalModels = 0
    local uuidModels = 0
    local nearbyModels = 0
    local modelsWithVisuals = 0
    
    print("🔍 НАЧИНАЮ СКАНИРОВАНИЕ WORKSPACE...")
    
    -- 🔥 ПРОСТАЯ И РАБОЧАЯ ЛОГИКА ИЗ v3.226 + ДИАГНОСТИКА
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            ...
            if obj.Name:find("%{") and obj.Name:find("%}") then
                ...
                print(string.format("🔍 UUID модель: %s", obj.Name))
                
                local success, modelCFrame = pcall(function() return obj:GetModelCFrame() end)
                if success then
                    local distance = (modelCFrame.Position - playerPos).Magnitude
                    print(string.format("  📍 Позиция: %s, расстояние: %.1f стадов", tostring(modelCFrame.Position), distance))
                    
                    if distance <= CONFIG.SEARCH_RADIUS then
                        nearbyModels = nearbyModels + 1
                        print(string.format("  ✅ В радиусе! Проверяю визуалы..."))
                        ...
                        if hasVisuals then
                            ...
                            print(string.format("🎉 ✅ НАЙДЕН ПИТОМЕЦ: %s (расстояние: %.1f)", obj.Name, distance))
                            print(string.format("   📊 MeshParts: %d (включая 0 - тоже питомец!)", meshes and #meshes or 0))
                            ...
                            print("  ⚠️ Нет признаков питомца (ни MeshParts, ни BaseParts)")
                        end
                    else
                        print(string.format("  ❌ Слишком далеко (%.1f > %d)", distance, CONFIG.SEARCH_RADIUS))
                    end
                else
                    print("  ❌ Ошибка GetModelCFrame()")
                end
            end
        end
    end
    ...
    print(string.format("📊 Всего моделей: %d", totalModels))
    print(string.format("🔍 UUID моделей: %d", uuidModels))
    print(string.format("🎯 В радиусе: %d", nearbyModels))
    print(string.format("👀 С визуалами: %d", modelsWithVisuals))
    print(string.format("🎉 Найдено питомцев: %d", #foundPets))
    
    if #foundPets == 0 then
        print("❌ ПИТОМЦЫ НЕ НАЙДЕНЫ!")
        if uuidModels == 0 then
            print("🚨 ПРОБЛЕМА: Ни одной UUID модели не найдено!")
            print("💡 Проверьте что питомец рядом и имеет имя в формате {UUID}")
        elseif nearbyModels == 0 then
            print("🚨 ПРОБЛЕМА: UUID модели есть, но все слишком далеко!")
            print("💡 Подойдите ближе к питомцу (меньше 100 стадов)")
        elseif modelsWithVisuals == 0 then
            print("🚨 ПРОБЛЕМА: UUID модели рядом, но у них нет визуалов!")
            print("💡 Возможно это не питомцы, а другие объекты")
        end
        return nil
    end
    ...
    print(string.format("🎯 ✅ ВЫБРАН ПИТОМЕЦ: %s (расстояние: %.1f)", targetPet.model.Name, targetPet.distance))
...
local function main()
    ...
    -- УБРАНО: Масштабирование и размещение рядом с оригиналом
    -- НОВОЕ: Копия сразу помещается в руки игрока с анимацией
    print("\n🤏 === РАЗМЕЩЕНИЕ В РУКЕ (БЕЗ МАСШТАБИРОВАНИЯ) ===")
    
    -- Возвращаем Tool вместо обычной модели
    return petTool
