...
local function deepCopyModel(originalModel)
    ...
    -- 🚨 МИНИМАЛЬНАЯ ФИЛЬТРАЦИЯ: Удаляем ТОЛЬКО Handle!
    local removedParts = {}
    for _, part in ipairs(copy:GetDescendants()) do
        if part:IsA("BasePart") then
            ...
            -- Удаляем ТОЛЬКО Handle - ОСТАЛЬНОЕ ОСТАВЛЯЕМ!
            if part.Name == "Handle" then
                shouldRemove = true
                reason = "Handle (не нужен для копии)"
            end
            
            -- 🚨 ВСЕ ОСТАЛЬНОЕ ОСТАВЛЯЕМ! НЕ УДАЛЯЕМ ЧАСТИ DRAGONFLY!
    ...
    if originalModel.PrimaryPart and copy.PrimaryPart then
        ...
        -- 🚨 КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: НЕ ПОМЕЩАЕМ КОПИЮ В TOOL!
        -- ОСТАВЛЯЕМ КОПИЮ КАК ОТДЕЛЬНУЮ MODEL ДЛЯ getAllParts!
        
        -- НЕ создаем Tool - оставляем копию как Model в Workspace
        print("🔧 ОСТАВЛЯЕМ копию как отдельную Model в Workspace")
        print("✅ Копия остается в Workspace для правильной работы getAllParts!")
        
        -- Позиционируем копию рядом с оригиналом
        local newPosition = originalPos + offset
        copy.PrimaryPart.CFrame = CFrame.new(newPosition)
        
        print("🚨 Оригинальная позиция:", originalPos)
        print("✅ Копия размещена рядом с оригиналом:", newPosition)
