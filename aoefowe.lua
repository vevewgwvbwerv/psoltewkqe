...
local function getAnimatedParts(tool)
    ...
    -- 🔍 Ищем все BasePart в Tool, включая подмодели
    for _, obj in pairs(tool:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Name ~= "Handle" then
            table.insert(parts, obj)
            print(string.format("📦 Найдена часть: %s (родитель: %s)", obj.Name, obj.Parent.Name))
        end
    end
    
    print(string.format("📊 Всего найдено анимируемых частей: %d", #parts))
    return parts
end
