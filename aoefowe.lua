...
-- 🎯 ФУНКЦИЯ СТАЦИОНАРНОЙ CFrame АНИМАЦИИ (ТОЛЬКО ВРАЩЕНИЯ, БЕЗ ПОЗИЦИИ)
local function startStationaryCFrameAnimation(originalModel, copyModel)
    ...
    for partName, copyPart in pairs(copyParts) do
        if copyPart and copyPart.Parent then
            initialPositions[partName] = copyPart.Position
        end
    end
    
    print("📍 Сохранил начальные позиции копии - она НЕ будет двигаться!")
    
    -- Запускаем стационарный анимационный цикл
    local connection = RunService.Heartbeat:Connect(function()
        local appliedCount = 0
        local changesDetected = 0
        ...
        for partName, originalPart in pairs(originalParts) do
            ...
            if copyPart and originalPart.Parent and copyPart.Parent and initialPos then
                local success, errorMsg = pcall(function()
                    local originalCFrame = originalPart.CFrame
                    
                    -- 🎯 КЛЮЧЕВОЕ ОТЛИЧИЕ: КОПИРУЕМ ТОЛЬКО ВРАЩЕНИЕ, НО ОСТАВЛЯЕМ ПОЗИЦИЮ НА МЕСТЕ
                    local newCFrame = CFrame.new(initialPos, initialPos + originalCFrame.LookVector) * 
                                     (originalCFrame - originalCFrame.Position)
                    
                    local hasChanged = copyPart.CFrame ~= newCFrame
                    if hasChanged then
                        changesDetected = changesDetected + 1
                    end
                    
                    -- Проверяем что CFrame не является NaN или бесконечностью
                    if newCFrame.Position.X ~= newCFrame.Position.X or 
                       math.abs(newCFrame.Position.X) == math.huge or
                       math.abs(newCFrame.Position.Y) == math.huge or
                       math.abs(newCFrame.Position.Z) == math.huge then
                        return -- Пропускаем некорректные значения
                    end
                    
                    -- 🎯 ПРИМЕНЯЕМ СТАЦИОНАРНЫЙ CFrame (ВРАЩЕНИЕ НА МЕСТЕ)
                    if copyPart.Parent then
                ...
                if not success then
                    print("❌ Ошибка при применении стационарного CFrame", partName, ":", errorMsg)
                end
            end
        end
        ...
        if tick() % 1 < 0.02 then -- Примерно каждую секунду
            print(string.format("🎯 Стационарная анимация: применено=%d, изменений=%d", 
