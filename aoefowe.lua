...
-- Функция мониторинга workspace.visuals
local function startVisualsMonitoring()
    print("\n👁️ === ЗАПУСК МОНИТОРИНГА WORKSPACE.VISUALS ===")
    
    local visuals = Workspace:FindFirstChild("visuals")
    if not visuals then
        print("⚠️ workspace.visuals не найден, создаю мониторинг для будущего появления...")
        
        -- Мониторим появление папки visuals
        local visualsConnection
        visualsConnection = Workspace.ChildAdded:Connect(function(child)
            if child.Name == "visuals" then
                print("✅ workspace.visuals появился! Запускаю мониторинг...")
                visualsConnection:Disconnect()
                startVisualsMonitoring() -- Рекурсивный вызов
            end
        end)
        return
    end
    
    print("✅ workspace.visuals найден! Начинаю мониторинг питомцев...")
    
    -- Мониторим появление новых питомцев в visuals
    local petConnection = visuals.ChildAdded:Connect(function(newPet)
        if newPet:IsA("Model") then
            print("\n🥚 === ОБНАРУЖЕН НОВЫЙ ПИТОМЕЦ В VISUALS ===")
            print("🐾 Имя питомца:", newPet.Name)
            
            -- Ждем немного, чтобы питомец полностью загрузился
            wait(0.1)
            
            -- Получаем позицию оригинального питомца
            local originalPosition
            if newPet.PrimaryPart then
                originalPosition = newPet.PrimaryPart.Position
            elseif newPet:FindFirstChild("RootPart") then
                originalPosition = newPet.RootPart.Position
            else
            ...
            if originalPosition then
                ...
                local animatedCopy = createAnimatedCopyAtPosition(newPet, originalPosition)
                ...
                if animatedCopy then
                    ...
                    if newPet.PrimaryPart then
                        newPet:SetPrimaryPartCFrame(newPet.PrimaryPart.CFrame - Vector3.new(0, 1000, 0))
                    end
    ...
    print("🔄 Мониторинг workspace.visuals активен!")
...
local function createGUI()
    ...
    autoButton.MouseButton1Click:Connect(function()
        if not autoReplaceActive then
            ...
            spawn(function()
                visualsConnection = startVisualsMonitoring()
