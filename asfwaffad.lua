            ...
            for _, handPart in ipairs(handPetParts) do
                if handPart and handPart.Parent then
                    ...
                    if copyPart and copyPart.Parent and not copyPart.Anchored then
                        ...
                        -- 🚨 ЭКСТРЕМАЛЬНАЯ ЗАЩИТА: ПОЛНОСТЬЮ ЗАПРЕЩАЕМ ЛЮБЫЕ ПЕРЕМЕЩЕНИЯ!
                        local originalPos = copyPart.Position  -- Сохраняем оригинальную позицию
                        
                        -- ОТКЛЮЧАЕМ ЛЮБУЮ АНИМАЦИЮ! ПИТОМЕЦ ОСТАЕТСЯ НА МЕСТЕ!
                        -- НИКАКИХ CFrame изменений, никаких Lerp, никаких поворотов!
                        
                        -- Принудительно возвращаем на оригинальную позицию
                        copyPart.Position = originalPos
                        copyPart.Anchored = true  -- ПОЛНОСТЬЮ замораживаем!
                        
                        -- Проверяем что позиция НЕ изменилась
                        if math.abs(copyPart.Position.Y - originalPos.Y) > 0.01 then
                            print("❌ КРИТИЧЕСКОЕ ПРЕДУПРЕЖДЕНИЕ: Часть", copyPart.Name, "пытается сдвинуться!")
                            print("  Оригинальная Y:", originalPos.Y, "Текущая Y:", copyPart.Position.Y)
