...
-- Практичная функция для проверки idle состояния
function isOriginalInStrictIdle(original)
    ...
    if velocity.Magnitude > 0.3 then -- Разумный порог
        return false
    end
    
    -- Практичные критерии движения
    if humanoid.MoveDirection.Magnitude > 0.05 then -- Разумный порог
        return false
    end
