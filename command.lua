...
-- Мониторинг рук персонажа
local function monitorCharacterTools()
    logEvent("SYSTEM", "Starting character tools monitoring")
...
-- Мониторинг Workspace для UUID питомцев
local function monitorWorkspacePets()
    logEvent("SYSTEM", "Starting workspace pets monitoring")
    
    Workspace.ChildAdded:Connect(function(child)
        if child:IsA("Model") and child.Name:find("{") then
            logEvent("WORKSPACE_PET_ADDED", child.Name, {
