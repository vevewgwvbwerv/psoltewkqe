...
local CONFIG = {
    ...
    -- Структурные параметры (из диагностики: 56 частей)
    STRUCTURE = {
        TOTAL_PARTS = 56,
        BASE_SIZE = Vector3.new(4, 4.5, 4), -- Размер основного яйца
        SHELL_THICKNESS = 0.2,
        MATERIAL = Enum.Material.Neon,
        BASE_COLOR = Color3.fromRGB(255, 200, 100), -- Золотистый цвет
        TRANSPARENCY = 0.1
    },
    
    -- Интерактивность (из диагностики: ProximityPrompt "Hatch!")
    INTERACTION = {
        ACTION_TEXT = "Hatch!",
        OBJECT_TEXT = "Egg",
...
function EggStructureBuilder:createEggEffects(centerPosition)
    ...
    table.insert(effects, lightPart)
    
    -- Создаем Attachment для эффектов частиц
    local mainAttachment = Instance.new("Attachment")
    mainAttachment.Name = "EggEffectsAttachment"
    mainAttachment.Position = Vector3.new(0, 0, 0)
    mainAttachment.Parent = lightPart
    
    -- Частицы магии вокруг яйца
    local magicParticles = Instance.new("ParticleEmitter")
    magicParticles.Name = "MagicParticles"
    magicParticles.Texture = "rbxasset://textures/particles/sparkles_main.dds"
    magicParticles.Lifetime = NumberRange.new(1.0, 3.0)
    magicParticles.Rate = 20
    magicParticles.SpreadAngle = Vector2.new(360, 360)
    magicParticles.Speed = NumberRange.new(2, 5)
    magicParticles.Color = ColorSequence.new{
    ...
    magicParticles.Parent = mainAttachment
    
    -- Аура вокруг яйца
    local auraPart = self:createBasePart("EggAura", CONFIG.STRUCTURE.BASE_SIZE * 1.5, centerPosition, {
        material = Enum.Material.ForceField,
    ...
    table.insert(effects, auraPart)
    
    -- Анимация ауры
    local auraAnimation = TweenService:Create(auraPart, TweenInfo.new(2, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
        Transparency = 0.9,
        Size = CONFIG.STRUCTURE.BASE_SIZE * 1.8
    })
    auraAnimation:Play()
    
    logger:info("✅ Создано %d эффектов", #effects)
    return effects, {auraAnimation}
end

function EggStructureBuilder:assembleEggModel(position)
    logger:info("Сборка модели яйца в позиции: %s", tostring(position))
    
    -- Создаем основную модель
    local eggModel = Instance.new("Model")
    eggModel.Name = CONFIG.EGG_NAME
    eggModel.Parent = Workspace
    
    -- Создаем части яйца
    local shellParts = self:buildEggShell(position)
    local effects, animations = self:createEggEffects(position)
    
    -- Добавляем все части в модель
    ...
    for _, effect in ipairs(effects) do
        effect.Parent = eggModel
        table.insert(self.parts, effect)
    end
    
    -- Устанавливаем PrimaryPart
    if #shellParts > 0 then
        eggModel.PrimaryPart = shellParts[1] -- Основное тело яйца
    end
    
    -- Создаем соединения между частями (WeldConstraints)
    if eggModel.PrimaryPart then
        for i = 2, #shellParts do
            local weld = Instance.new("WeldConstraint")
            weld.Part0 = eggModel.PrimaryPart
            weld.Part1 = shellParts[i]
            weld.Parent = eggModel.PrimaryPart
    ...
    logger:info("✅ Модель яйца собрана: %d частей, %d соединений", #self.parts, #self.welds)
    return eggModel
end
...
function EggStructureBuilder:addInteractivity(eggModel)
    ...
    hoverGui.Name = "EggHoverGui"
    hoverGui.Size = UDim2.new(0, 200, 0, 50)
    hoverGui.StudsOffset = Vector3.new(0, CONFIG.STRUCTURE.BASE_SIZE.Y + 2, 0)
    hoverGui.Parent = eggModel.PrimaryPart
    
    local hoverLabel = Instance.new("TextLabel")
    hoverLabel.Size = UDim2.new(1, 0, 1, 0)
    hoverLabel.BackgroundTransparency = 1
    hoverLabel.Text = "🥚 " .. CONFIG.INTERACTION.OBJECT_TEXT
    hoverLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
    hoverLabel.TextScaled = true
    hoverLabel.Font = Enum.Font.SourceSansBold
    hoverLabel.TextStrokeTransparency = 0
    hoverLabel.TextStrokeColor3 = Color3.fromRGB(0, 0, 0)
    hoverLabel.Parent = hoverGui
    
    -- Анимация текста
    local textAnimation = TweenService:Create(hoverLabel, TweenInfo.new(1, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true), {
...
function EffectsSystem:preloadSounds()
    ...
    logger:info("✅ Предзагружено %d звуков", #self.soundCache)
end

function EffectsSystem:createExplosionEffect(position, intensity)
    logger:info("💥 Создание эффекта взрыва в позиции: %s", tostring(position))
    ...
    -- Создаем осколки яйца
    for i = 1, 12 do
        local shard = Instance.new("Part")
        shard.Name = "EggShard_" .. i
        shard.Size = Vector3.new(0.5, 0.8, 0.3)
        shard.Position = position + Vector3.new(math.random(-2, 2), math.random(0, 3), math.random(-2, 2))
        ...
        -- Добавляем BodyVelocity для разлета осколков
        ...
        -- Анимация исчезновения осколков
...
function EffectsSystem:playSound(soundName, position, properties)
    ...
    -- Клонируем звук для воспроизведения
    local soundClone = sound:Clone()
    
    if properties then
        soundClone.Volume = properties.volume or soundClone.Volume
        soundClone.Pitch = properties.pitch or soundClone.Pitch
    end
    
    if position then
        -- Создаем временную часть для позиционного звука
