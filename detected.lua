...
local function main()
    ...
    -- КРИТИЧЕСКИ ВАЖНО: КОРРЕКЦИЯ ОРИЕНТАЦИИ ДО ЗАПУСКА АНИМАЦИИ
    print("\n🔧 === ПРИНУДИТЕЛЬНАЯ КОРРЕКЦИЯ ОРИЕНТАЦИИ ===")
    
    if petCopy.PrimaryPart then
        local currentPosition = petCopy.PrimaryPart.Position
        
        -- ПРИНУДИТЕЛЬНО создаем вертикальный CFrame (как у оригинала)
        local uprightCFrame = CFrame.new(
            currentPosition,
            currentPosition + Vector3.new(0, 0, -1)  -- Направление вперед
        )
        ...
        -- Применяем вертикальную ориентацию
        petCopy:SetPrimaryPartCFrame(uprightCFrame)
        
        print("✅ Копия принудительно поставлена в вертикальное положение!")
        
        -- Дополнительная коррекция всех частей
        local correctedParts = 0
        for _, part in pairs(petCopy:GetDescendants()) do
            if part:IsA("BasePart") and part ~= petCopy.PrimaryPart then
                local partCFrame = part.CFrame
                local upVector = partCFrame.UpVector
                
                -- Если часть повернута неправильно
                if math.abs(upVector.Y) < 0.7 then
                    local correctedPartCFrame = CFrame.new(
                        partCFrame.Position,
                        partCFrame.Position + Vector3.new(0, 0, -1)
                    )
        ...
        print("✅ Коррекция ориентации завершена! Исправлено частей:", correctedParts)
        print("🚀 Копия теперь стоит правильно - КАК ОРИГИНАЛ!")
    else
        print("⚠️ Нет PrimaryPart для коррекции ориентации")
    end
