...
-- Функция простого копирования модели (БЕЗ ПОЗИЦИОНИРОВАНИЯ)
local function deepCopyModel(originalModel)
    print("📋 Создаю простую копию модели для руки:", originalModel.Name)
    
    local copy = originalModel:Clone()
    copy.Name = originalModel.Name .. "_IN_HAND"
    -- НЕ помещаем в Workspace! Будет помещен в Handle!
    
    print("✅ Копия создана:", copy.Name)
    print("🚫 Убрано: позиционирование рядом с оригиналом")
    print("⚡ Копия готова для мгновенного помещения в руку!")
...
-- Главная функция v2.0 - МГНОВЕННОЕ ПОЯВЛЕНИЕ В РУКЕ
local function main()
    print("🚀 PetScaler v2.0 запущен! (МГНОВЕННЫЙ РЕЖИМ)")
    ...
    if not handle then
        print("❌ Handle не найден! Возьмите питомца в руку сначала.")
        return
    end
    
    print("✅ Handle найден:", handle.Name)
    
    -- Шаг 3: МГНОВЕННОЕ СОЗДАНИЕ КОПИИ В РУКЕ
    print("\n⚡ === МГНОВЕННОЕ СОЗДАНИЕ В РУКЕ ===")
    print("🚫 Убрано: создание рядом с оригиналом")
    print("🚫 Убрано: плавное масштабирование")
    print("✅ Питомец появляется МОМЕНТАЛЬНО в руке!")
    
    local petCopy = petModel:Clone()
    petCopy.Name = petModel.Name .. "_IN_HAND"
    petCopy.Parent = handle -- СРАЗУ В РУКУ!
    
    -- Настраиваем все части для анимации в руке
    for _, part in pairs(petCopy:GetDescendants()) do
        if part:IsA("BasePart") then
            part.Anchored = false -- Разрешаем анимацию
            part.CanCollide = false -- Убираем коллизии
        end
    end
    
    -- Исправляем Attachment связи
    fixAttachmentParenting(petCopy)
    
    -- Шаг 4: ЗАПУСК ЖИВОЙ АНИМАЦИИ В РУКЕ
    print("\n🔄 === ЗАПУСК АНИМАЦИИ В РУКЕ ===")
    print("⚡ Питомец моментально в руке с живой анимацией!")
    
    -- Запускаем копирование анимации с оригинала
    local endlessConnection = startEndlessIdleLoop(petModel, petCopy)
    
    if endlessConnection then
        print("🎉 === УСПЕХ! ===")
        print("✅ Питомец МОМЕНТАЛЬНО появился в руке!")
        print("✅ Живая анимация запущена")
        print("🚫 Убрано: плавное увеличивание рядом с оригиналом")
        print("⚡ Питомец сразу в руке с анимацией!")
        print("🔥 Копия всегда в движении - как живая!")
    else
        print("⚠️ Питомец в руке, но анимация не запустилась")
