...
if initSuccess then
    ...
    -- === АВТОМАТИЧЕСКИЙ МОНИТОРИНГ ПИТОМЦА В РУКАХ ===
    print("\n🔄 === ЗАПУСК АВТОМАТИЧЕСКОГО МОНИТОРИНГА ПИТОМЦА В РУКАХ ===")
    ...
    local autoHandConnection = RunService.Heartbeat:Connect(function()
        local player = Players.LocalPlayer
        ...
        if handTool then
            ...
            if isPet and not processedTools[handTool] then
                ...
                -- Автоматически вызываем функцию замены (КАК КНОПКА!)
                spawn(function()
                    local success = replaceHandPetWithAnimation()
                    if success then
    ...
    print("✅ Автоматический мониторинг питомца в руках запущен!")
    print("💡 Просто возьмите питомца в руки - замена произойдет сама!")
