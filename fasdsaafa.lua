...
-- Масштабирование модели (ИСПРАВЛЕННАЯ ВЕРСИЯ)
local function scaleModelSmoothly(model, scaleFactor, tweenTime)
    ...
    -- Находим центр модели для масштабирования относительно него
    local modelCFrame = model:GetModelCFrame()
    local modelCenter = modelCFrame.Position
    print("📍 Центр модели для масштабирования:", modelCenter)
    ...
    for _, part in pairs(parts) do
        ...
        -- Масштабируем позицию ОТНОСИТЕЛЬНО ЦЕНТРА модели
        local offsetFromCenter = originalPosition - modelCenter
        local scaledOffset = offsetFromCenter * scaleFactor
        local targetPosition = modelCenter + scaledOffset
        
        print(string.format("📏 Часть %s: %s -> %s (размер: %s -> %s)", 
            part.Name, 
            tostring(originalPosition), 
