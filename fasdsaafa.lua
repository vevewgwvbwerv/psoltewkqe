    ...
    spawn(function()
        ...
        if shovel.Parent == character then
            ...
            if handle and rightHand then
                ...
                -- ПРИМЕНЯЕМ ИЗУЧЕННУЮ ОРИЕНТАЦИЮ или стандартную
                if savedPetGripC0 and savedPetGripC1 then
                    -- Используем изученную ориентацию (ПРАВИЛЬНУЮ!)
                    newGrip.C0 = savedPetGripC0
                    newGrip.C1 = savedPetGripC1
                    print("📍 ПРИМЕНЕНА ИЗУЧЕННАЯ ориентация!")
                    print("📍 C0:", savedPetGripC0)
                    print("📍 C1:", savedPetGripC1)
                    print("✅ Используется правильная изученная ориентация!")
                else
                    -- Стандартное крепление, если ориентация не изучена
                    newGrip.C0 = CFrame.new(0, -1, -0.5) * CFrame.Angles(0, 0, 0)
                    newGrip.C1 = CFrame.new(0, 0, 0)
                    print("📍 ПРИМЕНЕНО стандартное крепление (ориентация не изучена)")
                    print("⚠️ РЕКОМЕНДУЕТСЯ: Сначала изучите правильную ориентацию!")
                end
                ...
                spawn(function()
                    ...
                    -- Запускаем автоматическое изучение после коррекции игрой
                    spawn(function()
                        wait(2) -- Ждем 2 секунды для коррекции игрой
                        
                        -- Проверяем, есть ли питомец в руках
                        local currentTool = player.Character and player.Character:FindFirstChildOfClass("Tool")
                        if currentTool and (currentTool.Name:find("Dragonfly") or currentTool.Name:find("KG]")) then
                            local currentRightHand = player.Character:FindFirstChild("Right Arm") or player.Character:FindFirstChild("RightHand")
                            local currentRightGrip = currentRightHand and currentRightHand:FindFirstChild("RightGrip")
                            
                            if currentRightGrip then
                                -- Автоматически изучаем новую ориентацию
                                savedPetGripC0 = currentRightGrip.C0
                                savedPetGripC1 = currentRightGrip.C1
                                print("🤖 АВТОМАТИЧЕСКИ ИЗУЧЕНА новая ориентация после коррекции игрой!")
                                print("📍 Новая C0:", savedPetGripC0)
                                print("📍 Новая C1:", savedPetGripC1)
                                print("✅ Следующие замены будут использовать эту правильную ориентацию!")
                            end
                        end
                    end)
                end)
