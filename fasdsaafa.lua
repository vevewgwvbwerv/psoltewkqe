    ...
    spawn(function()
        ...
        if shovel.Parent == character then
            ...
            if handle and rightHand then
                ...
                -- ПРИМЕНЯЕМ ПРАВИЛЬНУЮ ОРИЕНТАЦИЮ (ЖЕСТКО ЗАКОДИРОВАННУЮ)
                -- Используем точные значения для правильной ориентации питомца
                newGrip.C0 = CFrame.new(0, -1, 0, 1, 0, 0, 0, 0, 1, 0, -1, 0)
                newGrip.C1 = CFrame.new(-0.0670368001, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1)
                print("📍 ПРИМЕНЕНА ПРАВИЛЬНАЯ ориентация питомца!")
                print("📍 C0: CFrame.new(0, -1, 0, 1, 0, 0, 0, 0, 1, 0, -1, 0)")
                print("📍 C1: CFrame.new(-0.0670368001, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1)")
                print("✅ Питомец должен появиться в правильной позе!")
                ...
                -- КРИТИЧЕСКИ ВАЖНО: Защищаем наш Weld от перезаписи игрой (ВРЕМЕННО)
                weldProtectionActive = true
                spawn(function()
                    local protectionTime = 0
                    while newGrip and newGrip.Parent and weldProtectionActive and protectionTime < 50 do -- Защита только 0.5 секунды
                        ...
                        if gameGrip and gameGrip ~= newGrip then
                            print("🛡️ Обнаружен автоматический RightGrip игры - удаляем!")
                            gameGrip:Destroy()
                            ...
                            if not rightHand:FindFirstChild("RightGrip") then
                                ...
                                restoredGrip.C0 = CFrame.new(0, -1, 0, 1, 0, 0, 0, 0, 1, 0, -1, 0)
                                restoredGrip.C1 = CFrame.new(-0.0670368001, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0, 1)
                    ...
                    print("🛡️ Защита Weld отключена - игра может естественно корректировать ориентацию")
                end)
