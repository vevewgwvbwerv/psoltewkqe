...
-- 🔍 ПОИСК ОТДЕЛЬНОЙ МОДЕЛИ ПИТОМЦА В WORKSPACE
local function findWorkspacePetModel(toolName)
    ...
    print(string.format("🔍 Ищем модель для инструмента: %s", toolName))
    
    -- Ищем модели рядом с игроком (в радиусе 50 стадов)
    local nearbyPets = {}
    
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") then
            -- Проверяем UUID паттерн или название содержащее "Dog"
            local isUUIDModel = obj.Name:find("{") and obj.Name:find("}")
            local isDogModel = obj.Name:find("Dog") or obj.Name:find("dog")
            
            if isUUIDModel or isDogModel then
                -- Проверяем расстояние
                local success, modelCFrame = pcall(function() return obj:GetModelCFrame() end)
                if success then
                    local distance = (modelCFrame.Position - playerPos).Magnitude
                    if distance <= 50 then
                        -- Проверяем есть ли анимационные компоненты
                        local hasMotor6D = obj:FindFirstChildOfClass("Motor6D", true)
                        local hasAnimator = obj:FindFirstChildOfClass("Animator", true)
                        local hasHumanoid = obj:FindFirstChildOfClass("Humanoid", true)
                        
                        if hasMotor6D or hasAnimator or hasHumanoid then
                            table.insert(nearbyPets, {
                                model = obj,
                                distance = distance,
                                hasMotor6D = hasMotor6D,
                                hasAnimator = hasAnimator,
                                hasHumanoid = hasHumanoid
                            })
                            
                            print(string.format("  📦 Найдена модель: %s (%.1f стадов)", obj.Name, distance))
                            print(string.format("    Motor6D: %s, Animator: %s, Humanoid: %s",
                                hasMotor6D and "YES" or "NO",
                                hasAnimator and "YES" or "NO",
                                hasHumanoid and "YES" or "NO"))
                        end
                    end
                end
            end
    ...
    if #nearbyPets == 0 then
        print("❌ Анимированные модели питомцев рядом не найдены")
        return nil
    end
    
    -- Сортируем по расстоянию и берем ближайшего
    table.sort(nearbyPets, function(a, b) return a.distance < b.distance end)
    local closestPet = nearbyPets[1]
    
    print(string.format("🎯 Выбрана ближайшая модель: %s (%.1f стадов)", 
        closestPet.model.Name, closestPet.distance))
...
local function findHandPet()
    ...
    if handTool then
        ...
        -- Показываем структуру инструмента
        print("🔍 Структура инструмента:")
        for _, child in pairs(handTool:GetChildren()) do
            print(string.format("  - %s (%s)", child.Name, child.ClassName))
        end
        
        -- Ищем модель питомца в всех потомках (глубокий поиск)
        print("🔍 Поиск модели питомца в всех потомках...")
        for _, descendant in pairs(handTool:GetDescendants()) do
            if descendant:IsA("Model") then
                print(string.format("  📦 Модель: %s", descendant.Name))
                
                -- Проверяем UUID паттерн
                if descendant.Name:find("{") and descendant.Name:find("}") then
                    print("🐕 Найден питомец с UUID:", descendant.Name)
                    return descendant
                end
            end
        end
        
        -- Если UUID не найден, пробуем любую модель с анимациями
        print("🔍 Поиск любой модели с анимациями...")
        for _, descendant in pairs(handTool:GetDescendants()) do
            if descendant:IsA("Model") then
                -- Проверяем есть ли Motor6D или Animator
                local hasMotor6D = descendant:FindFirstChildOfClass("Motor6D", true)
                local hasAnimator = descendant:FindFirstChildOfClass("Animator", true)
                
                if hasMotor6D or hasAnimator then
                    print("🎭 Найдена анимированная модель:", descendant.Name)
                    print(string.format("  Motor6D: %s, Animator: %s", 
                        hasMotor6D and "YES" or "NO",
                        hasAnimator and "YES" or "NO"))
                    return descendant
                end
        ...
        print("⚠️ Модель питомца не найдена в инструменте")
        
        -- Поиск отдельной модели питомца в Workspace
        print("🔍 Поиск отдельной модели питомца в Workspace...")
        local petModel = findWorkspacePetModel(handTool.Name)
        if petModel then
            print("🎉 Найдена отдельная модель питомца!")
            return petModel
        end
    end
