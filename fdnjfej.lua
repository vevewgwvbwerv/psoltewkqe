    ...
    connection = RunService.Heartbeat:Connect(function()
        ...
        for key, originalMotor in pairs(originalMap) do
            ...
            if copyMotor and originalMotor.Parent then
                copyMotorStateFiltered(originalMotor, copyMotor, CONFIG.SCALE_FACTOR, original, copy)
            end
        end
...
local function main()
    ...
    -- Шаг 6: Простая блокировка позиции (фильтр анимаций блокирует walking)
    print("\n🔒 === ПРОСТАЯ БЛОКИРОВКА ПОЗИЦИИ ===")
    wait(0.5)
    
    -- Просто заякориваем корень - фильтр анимаций сделает остальное
    local copyRoot = petCopy:FindFirstChild("RootPart") or 
                     petCopy:FindFirstChild("Torso") or 
                     petCopy:FindFirstChild("HumanoidRootPart")
    
    if copyRoot then
        copyRoot.Anchored = true
        print("  ✅ Корень заякорен - фильтр анимаций блокирует walking движения")
    else
        print("  ⚠️ Корень не найден")
    end
