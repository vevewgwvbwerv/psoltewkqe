...
local function main()
    ...
    if originalRoot then
        ...
        if idleTrack then
            ...
            -- ⚡ МАКСИМАЛЬНО ЧАСТЫЙ МОНИТОРИНГ: КАЖДУЮ МИЛЛИСЕКУНДУ!
            spawn(function()
                -- Даем питомцу короткое время остановиться
                print("⏳ Даю 2 секунды для остановки...")
                wait(2)
                
                -- Сохраняем позицию
                local lockPosition = originalRoot.Position
                local lockCFrame = originalRoot.CFrame
                print("🔒 ЗАФИКСИРОВАЛ позицию:", lockPosition)
                
                -- МАКСИМАЛЬНО ЧАСТЫЙ МОНИТОРИНГ
                while idleTrack and idleTrack.Parent and petModel and petModel.Parent do
                    wait(0.01) -- КАЖДУЮ 0.01 секунды = 100 раз в секунду!
                    
                    -- 1. ПОСТОЯННО ПЕРЕЗАПУСКАЕМ IDLE
                    ...
                    -- 2. МОМЕНТАЛЬНО УНИЧТОЖАЕМ WALKING
                    ...
                    -- 3. ПОСТОЯННО ОБНУЛЯЕМ СКОРОСТЬ
                    if originalHumanoid then
                        originalHumanoid.WalkSpeed = 0
                        originalHumanoid.JumpPower = 0
                    end
                    
                    -- 4. МОМЕНТАЛЬНО ВОЗВРАЩАЕМ НА МЕСТО
                    if originalRoot then
                        local currentPos = originalRoot.Position
                        local distance = (currentPos - lockPosition).Magnitude
                        
                        if distance > 0.1 then -- ЛЮбое движение больше 0.1 студа
                            originalRoot.CFrame = lockCFrame
                            originalRoot.Velocity = Vector3.new(0, 0, 0)
                            originalRoot.AngularVelocity = Vector3.new(0, 0, 0)
                        end
                        
                        -- Обнуляем любую скорость
                        if originalRoot.Velocity.Magnitude > 0.01 then
                            originalRoot.Velocity = Vector3.new(0, 0, 0)
                            originalRoot.AngularVelocity = Vector3.new(0, 0, 0)
                        end
                    end
                    
                    -- 5. ПОСТОЯННО ПЕРЕУСТАНАВЛИВАЕМ IDLE НАСТРОЙКИ
                ...
                print("⚠️ Максимально частый мониторинг остановлен")
            end)
