...
local function main()
    ...
    -- 🎯 СИМУЛЯЦИЯ НАТИВНОЙ КОРРЕКЦИИ ROBLOX
    print("\n🎯 === СИМУЛЯЦИЯ НАТИВНОЙ КОРРЕКЦИИ ROBLOX ===")
    print("📊 Основано на анализе: питомец спавнится на Y=0.70, после движения Y=2.03")
    ...
    if petCopy.PrimaryPart then
        ...
        print("📊 Текущий UpVector:", currentCFrame.UpVector)
        
        -- ПРИМЕНЯЕМ ТОЧНУЮ КОРРЕКЦИЮ КАК ROBLOX
        -- Поднимаем Y-координату на +1.33 стада (как при первом движении)
        local correctedPosition = Vector3.new(
            currentPos.X,
            currentPos.Y + 1.33,  -- КЛЮЧЕВАЯ КОРРЕКЦИЯ: +1.33 стада вверх
            currentPos.Z
        )
        
        -- СОХРАНЯЕМ ОРИГИНАЛЬНУЮ ОРИЕНТАЦИЮ (как у оригинала)
        -- UpVector остается -0.00,-0.00,-1.00 - это нормально!
        local correctedCFrame = CFrame.new(
            correctedPosition,
            correctedPosition + currentCFrame.LookVector  -- Сохраняем направление
        ) * CFrame.Angles(currentCFrame:ToEulerAnglesXYZ())  -- Сохраняем углы
        
        -- Применяем коррекцию
        rootPart.CFrame = correctedCFrame
        
        print("✅ Применена нативная коррекция Roblox!")
        print("📊 Поднято на +1.33 стада вверх")
        ...
        print("📊 Новая позиция:", newPos)
        print("📊 Новый UpVector:", newUpVector)
        print("📊 Новый LookVector:", newLookVector)
        
        -- Проверка успешности коррекции
        local heightDifference = newPos.Y - currentPos.Y
        if math.abs(heightDifference - 1.33) < 0.1 then
            print("🎉 ОТЛИЧНО! Питомец поднят на правильную высоту!")
            print("📊 Разница в высоте: +", heightDifference, "стадов")
        else
            print("⚠️ Коррекция может быть неточной")
            print("📊 Фактическая разница: +", heightDifference, "стадов")
        end
