...
if initSuccess then
    ...
    local autoToolConnection = RunService.Heartbeat:Connect(function()
        ...
        if not player or not player.Character then 
            print("🔍 [DEBUG] Player или Character не найден")
            return 
        end
        
        local handTool = player.Character:FindFirstChildOfClass("Tool")
        if handTool then
            print("🔍 [DEBUG] Обнаружен Tool в руках:", handTool.Name)
            ...
            if handTool.Name:find("KG") or handTool.Name:find("Dragonfly") or 
               ...
               handTool.Name:find("%[") and handTool.Name:find("%]") and handTool.Name:find("Age") then
                isPet = true
                print("🔍 [DEBUG] Tool является питомцем!")
            else
                print("🔍 [DEBUG] Tool НЕ является питомцем:", handTool.Name)
            end
            ...
            elseif isPet and processedTools[handTool] then
                print("🔍 [DEBUG] Tool уже обработан ранее")
            end
        else
            print("🔍 [DEBUG] Tool в руках не найден")
        end
    end)
