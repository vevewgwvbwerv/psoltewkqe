...
local function startWorkspaceScanning()
    ...
    connection = RunService.Heartbeat:Connect(function()
        ...
        if math.floor(elapsed) % 5 == 0 and elapsed > 1 then -- Каждые 5 секунд
            ...
            -- Проверяем workspace.visuals (ИЩЕМ ПРОСТЫЕ ИМЕНА!)
            ...
            if visuals then
                ...
                for _, child in pairs(visuals:GetChildren()) do
                    if child:IsA("Model") then
                        ...
                        print("  🐾 Модель:", child.Name, "- Питомец:", isPet and "✅ ДА" or "❌ НЕТ")
                    end
                end
            else
        ...
        -- Шаг 1: ИЩЕМ ВИЗУАЛЬНОГО питомца в workspace.visuals (ПРОСТОЕ ИМЯ!)
        local visuals = Workspace:FindFirstChild("visuals")
        if visuals then
            for _, visualObj in pairs(visuals:GetChildren()) do
                if visualObj:IsA("Model") and not processedPetNames[visualObj.Name] then
                    ...
                    local objName = visualObj.Name:lower()
                    if objName == "golden lab" or objName == "bunny" or objName == "dog" or 
                       objName == "cat" or objName == "rabbit" or objName:find("lab") then
                        ...
                        print("🎭 НАЙДЕН визуальный питомец с простым именем:", visualObj.Name)
                        break
                    end
                end
            end
