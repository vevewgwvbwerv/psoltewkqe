    ...
    spawn(function()
        ...
        if shovel.Parent == character then
            ...
            if handle and rightHand then
                ...
                -- Используем точные значения из изученной правильной ориентации
                newGrip.C0 = CFrame.new(0, -1, 0) -- Позиция: 0, -1, 0
                newGrip.C1 = CFrame.new(-0.0670368001, 0, 0) -- Смещение: -0.0670368001, 0, 0
                
                print("📍 ПРИМЕНЕНА ПРАВИЛЬНАЯ ориентация питомца (ЖЕСТКО ЗАКОДИРОВАННАЯ)!")
                print("📍 C0: CFrame.new(0, -1, 0)")
                print("📍 C1: CFrame.new(-0.0670368001, 0, 0)")
                print("✅ Питомец должен появиться в правильной позе СРАЗУ!")
