    ...
    local hybridController = RunService.Heartbeat:Connect(function()
        ...
        elseif currentlyWalking and isRecording then
            ...
            if #recordedIdlePoses > 0 then
                ...
                -- РАДИКАЛЬНО: ПОЛНОСТЬЮ УНИЧТОЖАЕМ ANIMATOR!
                for _, obj in pairs(originalModel:GetDescendants()) do
                    if obj:IsA("Animator") then
                        -- СНАЧАЛА останавливаем все анимации
                        local tracks = obj:GetPlayingAnimationTracks()
                        for _, track in pairs(tracks) do
                            track:Stop()
                        ...
                        -- ЗАТЕМ ПОЛНОСТЬЮ УНИЧТОЖАЕМ ANIMATOR!
                        obj:Destroy()
                        print("💥 УНИЧТОЖИЛ ANIMATOR - теперь он не сможет перезаписывать наши idle позы!")
                        break
                    end
                end
            end
