...
local function main()
    ...
    -- 💪 РАДИКАЛЬНОЕ РЕШЕНИЕ: ПРИНУДИТЕЛЬНО ПОСТАВИМ НА НОГИ!
    print("\n💪 === РАДИКАЛЬНАЯ ПОСТАНОВКА НА НОГИ ===")
    
    if petCopy.PrimaryPart then
        local rootPart = petCopy.PrimaryPart
        local currentPos = rootPart.Position
        
        -- ШАГ 1: ОТКЛЮЧАЕМ ВСЕ Anchored для физики
        for _, part in pairs(petCopy:GetDescendants()) do
            if part:IsA("BasePart") then
                part.Anchored = false
                part.CanCollide = true  -- Включаем коллизии
            end
        end
        
        -- ШАГ 2: ПРИНУДИТЕЛЬНО ставим вертикально (КАК СОЛДАТИК!)
        local uprightCFrame = CFrame.new(
            Vector3.new(currentPos.X, currentPos.Y + 5, currentPos.Z),  -- Поднимаем на 5 стадов
            Vector3.new(currentPos.X, currentPos.Y, currentPos.Z + 1)   -- Направление вперед
        )
        
        rootPart.CFrame = uprightCFrame
        print("✅ Питомец поднят в воздух в вертикальном положении!")
        
        -- ШАГ 3: СОЗДАЕМ BodyPosition для удержания позиции
        local bodyPos = Instance.new("BodyPosition")
        bodyPos.MaxForce = Vector3.new(4000, 4000, 4000)
        bodyPos.Position = Vector3.new(currentPos.X, currentPos.Y, currentPos.Z)
        bodyPos.D = 2000  -- Демпфирование
        bodyPos.P = 10000 -- Мощность
        bodyPos.Parent = rootPart
        
        -- ШАГ 4: СОЗДАЕМ BodyAngularVelocity для стабилизации ориентации
        ...
        print("✅ Физические ограничения применены!")
        
        -- ШАГ 5: ОЖИДАЕМ ПОКА ПИТОМЕЦ ОПУСТИТСЯ НА ЗЕМЛЮ
        print("🔽 Ожидание пока питомец опустится на землю...")
        
        -- Ожидаем 3 секунды пока он опустится
        wait(3)
        
        -- ШАГ 6: УДАЛЯЕМ физические ограничения
        if bodyPos then bodyPos:Destroy() end
        if bodyAngular then bodyAngular:Destroy() end
        
        -- ШАГ 7: ЗАКРЕПЛЯЕМ ТОЛЬКО КОРНЕВУЮ ЧАСТЬ
        rootPart.Anchored = true
        
        -- Проверяем результат
        local finalUpVector = rootPart.CFrame.UpVector
        print("📊 Финальный UpVector:", finalUpVector)
        
        if finalUpVector.Y > 0.8 then
            print("🎉 УСПЕХ! Питомец стоит на ногах!")
        else
            print("⚠️ Питомец все еще может быть наклонен")
        end
        
        print("🚀 РАДИКАЛЬНАЯ ПОСТАНОВКА ЗАВЕРШЕНА!")
    else
        print("❌ Нет PrimaryPart у копии для радикальной постановки!")
    end
