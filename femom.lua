    ...
    if originalRoot then
        ...
        if originalAnimator then
            ...
            if idleTrack then
                ...
                -- АГРЕССИВНЫЙ МОНИТОРИНГ: принудительное зацикливание
                spawn(function()
                    while originalAnimator and originalAnimator.Parent and originalRoot and originalRoot.Parent do
                        wait(0.5) -- Проверяем каждые 0.5 секунд
                        
                        -- Останавливаем ВСЕ walking анимации
                        for _, track in pairs(originalAnimator:GetPlayingAnimationTracks()) do
                            ...
                            if trackName:find("walk") or trackName:find("run") or trackName:find("move") then
                                track:Stop()
                                print("🚫 ПРИНУДИТЕЛЬНО остановил ходьбу:", track.Name)
                            end
                        end
                        
                        -- Проверяем что idle анимация играет
                        if idleTrack and idleTrack.Parent then
                            if not idleTrack.IsPlaying then
                                idleTrack.Looped = true
                                idleTrack:Play()
                                print("🔄 ПРИНУДИТЕЛЬНО перезапустил idle!")
                            end
                        end
                        
                        -- Проверяем что оригинал остается заякорен
                        if originalRoot and not originalRoot.Anchored then
                            originalRoot.Anchored = true
                            print("⚓ ПРИНУДИТЕЛЬНО перезаякорил!")
                        end
                        
                        -- Проверяем что AI остается отключен
                        if originalHumanoid then
                            if originalHumanoid.WalkSpeed ~= 0 then
                                originalHumanoid.WalkSpeed = 0
                                originalHumanoid.JumpPower = 0
                                print("🤖 ПРИНУДИТЕЛЬНО переотключил AI!")
                            end
                        end
                    end
                ...
                print("✅ АГРЕССИВНЫЙ мониторинг idle запущен!")
                print("✅ Оригинал будет ПРИНУДИТЕЛЬНО анимироваться idle!")
            else
