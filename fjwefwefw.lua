...
-- ИДЕАЛЬНЫЙ ФИЛЬТР: Копируем ТОЛЬКО когда оригинал стоит (IDLE-ONLY)
local function copyMotorStateFiltered(originalMotor, copyMotor, scaleFactor, original, copy)
    ...
    if originalRoot then
        ...
        local speed = math.sqrt(velocity.X^2 + velocity.Z^2) -- Горизонтальная скорость
        
        -- РАДИКАЛЬНЫЙ ФИЛЬТР: Если движется - НЕ копируем вообще!
        if speed > 0.5 then
            return false -- Оставляем копию в последней idle позе
        end
    end
    
    -- Копируем Motor6D состояние ТОЛЬКО когда оригинал стоит
