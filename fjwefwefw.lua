...
local function main()
    ...
    -- 🧮 МАТЕМАТИЧЕСКОЕ РЕШЕНИЕ: ПРАВИЛЬНЫЙ ВЕРТИКАЛЬНЫЙ CFrame
    print("\n🧮 === МАТЕМАТИЧЕСКАЯ КОРРЕКЦИЯ ОРИЕНТАЦИИ ===")
    
    if petCopy.PrimaryPart then
        local rootPart = petCopy.PrimaryPart
        local currentPos = rootPart.Position
        
        print("📊 Текущая позиция:", currentPos)
        print("📊 Текущий UpVector:", rootPart.CFrame.UpVector)
        
        -- МАТЕМАТИЧЕСКИ ПРАВИЛЬНЫЙ вертикальный CFrame
        -- Используем CFrame.lookAt для правильной ориентации
        local uprightCFrame = CFrame.lookAt(
            currentPos,  -- Позиция
            currentPos + Vector3.new(0, 0, -1),  -- Направление (вперед)
            Vector3.new(0, 1, 0)  -- Верх (строго вверх!)
        )
        
        -- Применяем правильную ориентацию
        rootPart.CFrame = uprightCFrame
        
        print("✅ Применен математически правильный вертикальный CFrame!")
        
        -- Проверяем результат
        local newUpVector = rootPart.CFrame.UpVector
        local newLookVector = rootPart.CFrame.LookVector
        
        print("📊 Новый UpVector:", newUpVector)
        print("📊 Новый LookVector:", newLookVector)
        
        -- Проверка что UpVector действительно направлен вверх
        if math.abs(newUpVector.Y - 1) < 0.1 then
            print("🎉 ОТЛИЧНО! UpVector почти идеально направлен вверх!")
        elseif newUpVector.Y > 0.7 then
            print("✅ ХОРОШО! UpVector направлен преимущественно вверх")
        else
            print("⚠️ ПРОБЛЕМА! UpVector все еще не направлен вверх")
            
            -- Попытка альтернативной коррекции
            local alternativeCFrame = CFrame.new(currentPos) * CFrame.Angles(0, 0, 0)  -- Нулевые углы
            rootPart.CFrame = alternativeCFrame
            print("🔧 Применена альтернативная коррекция (нулевые углы)")
            
            local finalUpVector = rootPart.CFrame.UpVector
            print("📊 Финальный UpVector после альтернативной коррекции:", finalUpVector)
        end
        
        print("🚀 Математическая коррекция завершена!")
    else
        print("❌ Нет PrimaryPart у копии для математической коррекции!")
    end
