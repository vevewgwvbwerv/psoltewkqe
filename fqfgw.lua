        ...
        Buttons = {
            {
                ...
                Icon = "copy",
                Callback = function()
                    -- Попытка копирования с разбиением на части если текст слишком длинный
                    local maxClipboardSize = 50000
                    if #detailText > maxClipboardSize then
                        -- Копируем только первую часть и предупреждаем
                        local truncatedText = string.sub(detailText, 1, maxClipboardSize) .. "\n\n-- TEXT TRUNCATED DUE TO LENGTH LIMIT --\n-- Full data available in console output --"
                        pcall(function()
                            if setclipboard then
                                setclipboard(truncatedText)
                            else
                                game:GetService("GuiService"):SetClipboard(truncatedText)
                            end
                        end)
                        
                        WindUI:Notify({
                            Title = "Text Truncated",
                            Content = string.format("Text too long (%d chars). Copied first %d chars. Full data in console.", #detailText, maxClipboardSize),
                            Icon = "alert-triangle",
                            Duration = 5
                        })
                    else
