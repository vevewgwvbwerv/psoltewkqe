...
local function replaceToolInHand(newToolData)
    ...
    print("🔄 Создаю точную копию проанализированного Tool...")
    
    -- Создаем точную копию текущего Tool
    local newTool = deepCopyTool(currentTool)
    if not newTool then
        print("❌ Не удалось создать копию Tool")
        return false
    end
    
    -- Меняем имя на Dragonfly
    newTool.Name = "Dragonfly [6.36 KG] [Age 35]"
    
    print("📝 Структура скопированного Tool:")
    ...
    currentTool:Destroy()
    wait(0.1) -- Небольшая задержка для корректного удаления
    newTool.Parent = character
    
    print("✅ Tool успешно заменен на точную копию с именем Dragonfly!")
    return true
end
