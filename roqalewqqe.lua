...
local function main()
    ...
    if originalRoot then
        ...
        if originalAnimator then
            print("🎭 Нашел Animator оригинала - попытаемся зациклить idle!")
            ...
            for _, track in pairs(originalAnimator:GetPlayingAnimationTracks()) do
                if track.Name:lower():find("walk") or track.Name:lower():find("run") then
                    track:Stop()
                    print("💫 Остановил анимацию ходьбы:", track.Name)
                end
            end
            
            -- Пытаемся найти и запустить idle анимацию
            for _, track in pairs(originalAnimator:GetPlayingAnimationTracks()) do
                if track.Name:lower():find("idle") or track.Name:lower():find("stand") then
                    track.Looped = true -- Зацикливаем!
                    track:Play()
                    print("🔄 Зациклил idle анимацию:", track.Name)
                    break
                end
            end
            
            -- Отключаем AI чтобы он не переключался на ходьбу
            if originalHumanoid then
                originalHumanoid.WalkSpeed = 0
                originalHumanoid.JumpPower = 0
                print("🤖 AI отключен - не будет переключаться на ходьбу")
            end
        else
