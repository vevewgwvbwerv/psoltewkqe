...
local function properReplace()
    ...
    -- ИСПРАВЛЕНИЕ: Добавляем через Backpack для стабильности
    print("📦 Добавляю в Backpack сначала...")
...
local function startProperAnimation(tool)
    ...
    print(string.format("🎭 Найдено %d Motor6D для анимации", motorCount))
    
    if motorCount == 0 then
        print("⚠️ Motor6D не найдены - запускаю простую анимацию")
        startSimpleAnimation(tool)
        return
    end
...
-- ПРОСТАЯ АНИМАЦИЯ если Motor6D не найдены
local function startSimpleAnimation(tool)
    if not tool then return end
    
    print("🎬 === ЗАПУСК ПРОСТОЙ АНИМАЦИИ ===")
    ...
    -- Находим все BasePart (кроме Handle)
    local parts = {}
    local partCount = 0
    for _, obj in pairs(tool:GetDescendants()) do
        if obj:IsA("BasePart") and obj.Name ~= "Handle" then
            ...
            print("   📦 Найден Part: " .. obj.Name)
        end
    end
    
    print(string.format("🎭 Найдено %d частей для простой анимации", partCount))
    
    if partCount == 0 then
        print("⚠️ Части не найдены - анимация недоступна")
        return
    end
    
    -- Запускаем простую анимацию через CFrame (осторожно)
    animationConnection = RunService.Heartbeat:Connect(function()
        local time = tick()
        ...
        for partName, data in pairs(parts) do
            if data.part and data.part.Parent then
                local success, err = pcall(function()
                    -- Очень легкая анимация
                    local offsetY = math.sin(time * 1 + data.time) * 0.02
                    local offsetX = math.cos(time * 0.8 + data.time) * 0.01
                    
                    -- Применяем минимальную анимацию
                    local newCFrame = data.originalCFrame * CFrame.new(offsetX, offsetY, 0)
                    data.part.CFrame = newCFrame
                end)
                
                if not success then
                    print("⚠️ Ошибка простой анимации части " .. partName)
                end
                
                data.time = data.time + 0.005
            end
        end
    end)
    
    print("✅ Простая анимация запущена!")
    print("🎭 Питомец анимируется через CFrame (осторожно)")
end
